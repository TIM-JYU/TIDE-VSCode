...
describe('Tide', () => {
  ...
  test('downloadTaskSet should call runAndHandle', async () => {
    ...
    const runStub = sinon
      .stub(Tide as any, 'runAndHandle')
      .returns(new Promise((resolve) => resolve('mock data')))
    // Act
    ...
    sinon.assert.calledWith(
      runStub,
      sinon.match.array.startsWith(['task', 'create', 'test/path', '-a', '-d']),
    )
  })
  test('login should call runAndHandle and login successfully', async () => {
    ...
    const runStub = sinon
      .stub(Tide as any, 'runAndHandle')
      .callsFake(async (...args: unknown[]): Promise<void> => {
    ...
    assert.deepStrictEqual(
      result,
      { isLogged: true },
      'The login result should match the expected response',
    )
  })
})
